...
d = d[ d['County'].isin(['ADA','ELMORE', 'GEM','PAYETTE','CANYON','WASHINGTON']) ]
...
cropmixtable.replace(np.nan,0,inplace=True)
# Save Cropmix file
cropmixtable.to_csv(r'D:\TreasureValley\data\landuse\NASS_cropmix.csv',
                    encoding= 'utf-8')
...
#
#cropmixTV.loc[set(np.arange(1986,2016,3))].plot(kind='bar',stacked=True,colormap='tab20')			
#plt.legend(ncol=3,loc='lower right', bbox_to_anchor=(0.9,1.02))
#ax=plt.gca()
#plt.ylabel('Crop fraction',size=18)
#plt.xlabel('')
#ax.set_yticklabels(ax.get_ymajorticklabels(),size=14)
